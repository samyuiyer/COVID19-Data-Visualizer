...
public class Graph extends DisplayMode {
	...
	private void updateChart() {
		...
		if (scopeName != null) {
			if (scopeName.equals(SCOPE_NAMES[0])) {
				...
				d = countryBox.getValue() == null || countryBox.getValue().dataArray[5].equals("INVALID") ? d
						: countryBox.getValue();
	...
	private void setupSettings() {
		...
		ChangeListener<DataPoint> boxListener = new ChangeListener<DataPoint>() {
			@Override
			public void changed(@SuppressWarnings("rawtypes") ObservableValue ov, DataPoint t, DataPoint t1) {
				updateChart();
			}
		};
		...
		StringConverter<DataPoint> dataToString = new StringConverter<DataPoint>() {
			...
			public DataPoint fromString(String string) {
				...
				DataPoint invalid = new DataPoint(string, new String[] { "INVALID", "", "", "", "0", "INVALID" },
		...
		try {
			cityBox.setItems(FXCollections
					.observableList(filter(dm.at.suggest(cityBox.getEditor().getText()), true, false, false)));
			...
			stateBox.setItems(FXCollections
					.observableList(filter(dm.at.suggest(stateBox.getEditor().getText()), false, true, false)));
			...
			countryBox.setItems(FXCollections
					.observableList(filter(dm.at.suggest(countryBox.getEditor().getText()), false, false, true)));
		...
		settings.getChildren().addAll(timeSlider, sliderLabel, sliderStart, sliderEnd, rangeLabel, scopeLabel,
				globalRadio, countryRadio, countryBox, stateRadio, stateBox, cityRadio, cityBox, dataLabel, confRadio,
				deadRadio, recovRadio);
	}
